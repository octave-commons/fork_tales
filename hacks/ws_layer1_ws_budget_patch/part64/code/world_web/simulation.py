...
def _update_stream_motion_overlays(
    ...
    *,
    ...
    if isinstance(policy_obj, dict) and policy_obj:
        # Tick governor signals (small, safe to include in payloads)
        tick_signals = {
            'tick_budget_ms': float(_safe_float(policy_obj.get('tick_budget_ms', 0.0), 0.0)),
            'slack_ms': float(_safe_float(policy_obj.get('slack_ms', 0.0), 0.0)),
            'ingestion_pressure': float(_safe_float(policy_obj.get('ingestion_pressure', 0.0), 0.0)),
            'guard_mode': str(policy_obj.get('guard_mode', '') or ''),
            'ws_particle_max': int(_safe_float(policy_obj.get('ws_particle_max', 0.0), 0.0)),
        }
    ...
    if policy_obj:
        ...
        tick_budget_ms = _safe_float(policy_obj.get('tick_budget_ms', float('nan')), float('nan'))
        ingestion_pressure = _clamp01(_safe_float(policy_obj.get('ingestion_pressure', 0.0), 0.0))
        ws_particle_max = int(_safe_float(policy_obj.get('ws_particle_max', 0.0), 0.0))
