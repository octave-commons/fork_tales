...
class WorldHandler(BaseHTTPRequestHandler):
    ...
    def _handle_websocket(
        ...
        *,
        ...
        # WebSocket tick governor thresholds (slack gating for expensive payloads)
        ws_full_snapshot_min_slack_ms = float(
            _safe_float(os.getenv('SIM_WS_FULL_SNAPSHOT_MIN_SLACK_MS', '12') or '12', 12.0)
        )
        ws_graph_pos_max_default = int(
            max(64, _safe_float(os.getenv('SIM_WS_GRAPH_POS_MAX', '4096') or '4096', 4096.0))
        )
                    ...
                    if use_cached_snapshots:
                        ...
                        if cache_refresh_due:
                            ...
                            if (
                                ...
                                # Trim streamed particles before full snapshot broadcast to avoid WS blowups
                                # Also gate full snapshots behind slack to keep 12 TPS stable.
                                full_snapshot_budget_ok = tick_slack_ms() >= ws_full_snapshot_min_slack_ms
                                inbox_active = bool(_RUNTIME_INBOX_SYNC_LOCK.locked())
                                if full_snapshot_budget_ok and (not inbox_active):
                                    dyn = simulation_payload.get('presence_dynamics', {})
                                    if isinstance(dyn, dict):
                                        # hard cap particle stream
                                        if isinstance(dyn.get('field_particles'), list):
                                            dyn['field_particles'] = list(dyn.get('field_particles', [])[:_SIMULATION_WS_STREAM_PARTICLE_MAX])
                                        # cap graph positions (can be huge)
                                        gpos = dyn.get('graph_node_positions')
                                        if isinstance(gpos, dict) and len(gpos) > ws_graph_pos_max_default:
                                            capped = {}
                                            for k, v in gpos.items():
                                                capped[k] = v
                                                if len(capped) >= ws_graph_pos_max_default:
                                                    break
                                            dyn['graph_node_positions'] = capped
                                            simulation_payload['graph_node_positions_truncated'] = True
                                            simulation_payload['graph_node_positions_total'] = len(gpos)
                                        # drop heavy grids when under time pressure
                                        if tick_slack_ms() < (ws_full_snapshot_min_slack_ms + 4.0):
                                            dyn.pop('nooi_field', None)
                                    simulation_payload['presence_dynamics'] = dyn
                                    ...
                                    # Defer full snapshot; delta stream continues.
                                    pass
                                last_simulation_full_broadcast = now_monotonic
                                last_projection_delta_broadcast = now_monotonic
                                last_simulation_for_delta = simulation_delta_payload
                        ...
                        # Ingestion pressure: lock state + backlog + accelerator utilization
                        inbox_active = bool(_RUNTIME_INBOX_SYNC_LOCK.locked())
                        inbox_state = (
                            catalog.get('eta_mu_inbox', {}) if isinstance(catalog, dict) else {}
                        )
                        inbox_pending = int(_safe_float(inbox_state.get('pending_count', 0), 0.0))
                        inbox_pending_soft = max(1.0, float(os.getenv('ETA_MU_INBOX_PENDING_SOFT', '64') or '64'))
                        inbox_pending_pressure = _clamp01(inbox_pending / inbox_pending_soft)

                        dynamics = simulation_payload.get('presence_dynamics', {})
                        resource_heartbeat = dynamics.get('resource_heartbeat', {}) if isinstance(dynamics, dict) else {}
                        resource_devices = resource_heartbeat.get('devices', {}) if isinstance(resource_heartbeat, dict) else {}
                        gpu1 = resource_devices.get('gpu1', {}) if isinstance(resource_devices, dict) else {}
                        gpu2 = resource_devices.get('gpu2', {}) if isinstance(resource_devices, dict) else {}
                        npu0 = resource_devices.get('npu0', {}) if isinstance(resource_devices, dict) else {}
                        gpu_util = max(
                            _safe_float(gpu1.get('utilization', 0.0), 0.0),
                            _safe_float(gpu2.get('utilization', 0.0), 0.0),
                        ) / 100.0
                        npu_util = _safe_float(npu0.get('utilization', 0.0), 0.0) / 100.0

                        ingestion_pressure = _clamp01(
                            max(1.0 if inbox_active else 0.0, inbox_pending_pressure, gpu_util, npu_util)
                        )

                        # Estimate slack before sim work (used for policy + streaming throttle)
                        slack_ms_before_sim = tick_slack_ms()

                        # Dynamic stream cap under pressure to avoid WS blowups
                        ws_particle_max = int(_SIMULATION_WS_STREAM_PARTICLE_MAX)
                        ...
                        # Cap graph position payload size (avoid WS blowups on huge graphs)
                        ...
                        if (
                            ...
                            gpos = graph_node_positions
                            if isinstance(gpos, dict) and len(gpos) > graph_pos_cap:
                                capped = {}
                                for k, v in gpos.items():
                                    capped[k] = v
                                    if len(capped) >= graph_pos_cap:
                                        break
                                dynamics_patch["graph_node_positions"] = capped
                                tick_patch["graph_node_positions_truncated"] = True
                                tick_patch["graph_node_positions_total"] = len(gpos)
                            else:
                                dynamics_patch["graph_node_positions"] = gpos
                            tick_changed_keys.append(
                        ...
                        if (
                            ...
                            apos = presence_anchor_positions
                            if isinstance(apos, dict) and len(apos) > max(64, min(graph_pos_cap, 2048)):
                                cap2 = max(64, min(graph_pos_cap, 2048))
                                capped = {}
                                for k, v in apos.items():
                                    capped[k] = v
                                    if len(capped) >= cap2:
                                        break
                                dynamics_patch["presence_anchor_positions"] = capped
                                tick_patch["presence_anchor_positions_truncated"] = True
                                tick_patch["presence_anchor_positions_total"] = len(apos)
                            else:
                                dynamics_patch["presence_anchor_positions"] = apos
                            tick_changed_keys.append(
