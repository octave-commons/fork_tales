...
| `GET /v1/openai/codex/accounts` | List runtime OpenAI Codex accounts and cycle config |
| `POST /v1/openai/codex/login` | Add an OpenAI API key to the Codex rotation pool |
| `DELETE /v1/openai/codex/accounts/{account_id}` | Remove an OpenAI account from runtime and optional `.env` |
| `POST /v1/openai/codex/cycle` | Update OpenAI cycle strategy (`balanced` or `sequential`) |
| `GET /v1/openai/codex/events` | View append-only Codex account lifecycle events |
...
>
> **Auth:** management endpoints require the same proxy auth header as completion routes, for example `Authorization: Bearer <PROXY_API_KEY>`.

### OpenAI Session Affinity (Codex-safe multi-account)

For OpenAI traffic, the proxy can pin each `session_id` / `conversation_id` to one credential so concurrent sessions do not hop accounts mid-thread (which helps preserve server-side cache locality and avoids cross-account session mixing).

- Session IDs are read from request payload keys (`metadata.session_id`, `metadata.conversation_id`, `session_id`, `conversation_id`, `thread_id`, etc.) or headers (`session_id`, `x-session-id`, `conversation_id`, `x-conversation-id`).
- When a session ID is present, the proxy forwards session headers upstream (`session_id`, `conversation_id`, `x-session-id`, `x-conversation-id`) and keeps a sticky credential mapping per session.
- If a credential is removed or starts failing and rotation occurs, that session mapping is cleared and rebound on the next successful credential.
- Tuning environment variables:
  - `OPENAI_SESSION_AFFINITY_ENABLED=true|false` (default: `true`)
  - `OPENAI_SESSION_AFFINITY_MAX_ENTRIES=2000`
  - `OPENAI_SESSION_AFFINITY_IDLE_TTL_SECONDS=7200`
...
| [Smart Gateway Guide](SMART_GATEWAY.md) | Field-aware model router + React control panel |
| [TensorFlow Training Harness](training/tf_qwen3_vl/README.md) | Custom TensorFlow training loop scaffold for Qwen3-VL implementation |
